...
function speak(letter: string) {
  const speech = globalThis as typeof globalThis & {
    speechSynthesis?: { cancel: () => void; speak: (utterance: { text: string; lang: string }) => void };
...
function LetterTile({ letter, index, used, onDrop, onSelect }: LetterTileProps) {
  const position = useRef(new Animated.ValueXY()).current;
  const panResponder = useMemo(
    () =>
      PanResponder.create({
        onStartShouldSetPanResponder: () => !used,
        onMoveShouldSetPanResponder: (_, gesture) => !used && (Math.abs(gesture.dx) > 4 || Math.abs(gesture.dy) > 4),
  ...
  return (
    <Animated.View style={[styles.tileWrapper, { transform: position.getTranslateTransform(), opacity: used ? 0.35 : 1 }]}>
      <Pressable
...
export function WordBuilderGame() {
  ...
  const [feedback, setFeedback] = useState('Arrastra una letra al espacio brillante. También puedes tocarla.');
  ...
  const placeTile = useCallback(
    (tileIndex: number) => {
      ...
      if (selectedLetter !== targetLetter) {
        ...
        setFeedback(`Escuchaste ${selectedLetter}. Busca la letra ${targetLetter}.`);
        return;
      }
      ...
      if (nextPlacedLetters.length === level.word.length) {
        ...
        speak(level.word);
        setFeedback(`¡Excelente! Formaste ${level.word} con las letras ${level.word.split('').join(', ')}.`);
      } else {
  ...
  const handleDrop = useCallback(
    (tileIndex: number, pageX: number, pageY: number) => {
      ...
      const droppedOnTarget = zone && pageX >= zone.x && pageX <= zone.x + zone.width && pageY >= zone.y && pageY <= zone.y + zone.height;
  ...
  return (
    <View style={styles.game}>
      <View style={styles.progressRow}>
        ...
        <Text style={styles.progressText}>Palabra {levelIndex + 1} de {wordLevels.length}</Text>
      </View>

      <View style={styles.pictureCard}>
        <Text accessibilityLabel={level.word} style={styles.emoji}>{level.emoji}</Text>
        <Text style={styles.hint}>{level.hint}</Text>
      </View>

      <Text accessibilityLiveRegion="polite" style={styles.instruction}>{feedback}</Text>
        ...
        {level.word.split('').map((letter, index) => {
          ...
          return (
            <View
              ...
              style={[styles.slot, isTarget && styles.targetSlot, placedLetter && styles.filledSlot]}>
      ...
      {isComplete ? (
        <Pressable accessibilityRole="button" onPress={advanceLevel} style={styles.nextButton}>
          <Text style={styles.nextButtonText}>Siguiente palabra ✨</Text>
        </Pressable>
      ) : (
        <View style={styles.tileGrid}>
          {level.letters.map((letter, index) => (
            <LetterTile key={`${letter}-${index}`} index={index} letter={letter} used={usedTiles.includes(index)} onDrop={handleDrop} onSelect={placeTile} />
          ))}
        </View>
      )}
    </View>
...
const styles = StyleSheet.create({
  ...
  levelPill: { backgroundColor: '#E8DDFF', borderRadius: 999, color: '#522B8E', fontSize: 14, fontWeight: '800', overflow: 'hidden', paddingHorizontal: 14, paddingVertical: 7 },
  progressText: { color: '#655B73', fontSize: 14, fontWeight: '700' },
  pictureCard: { alignItems: 'center', backgroundColor: '#FFF7D9', borderRadius: 28, padding: 20 },
  emoji: { fontSize: 88 },
  hint: { color: '#473D51', fontSize: 17, fontWeight: '700', marginTop: 8, textAlign: 'center' },
  instruction: { color: '#4E4260', fontSize: 16, fontWeight: '600', lineHeight: 22, minHeight: 44, textAlign: 'center' },
  wordSlots: { flexDirection: 'row', gap: 10, justifyContent: 'center' },
  slot: { alignItems: 'center', backgroundColor: '#F4F0F8', borderColor: '#D5CBDD', borderRadius: 16, borderStyle: 'dashed', borderWidth: 2, height: 62, justifyContent: 'center', width: 56 },
  targetSlot: { backgroundColor: '#FFF0A8', borderColor: '#F0A800', transform: [{ scale: 1.06 }] },
  filledSlot: { backgroundColor: '#DFF8E7', borderColor: '#3BAF68', borderStyle: 'solid' },
  ...
  letterTile: { alignItems: 'center', backgroundColor: '#7A4CC2', borderBottomColor: '#4D2789', borderBottomWidth: 5, borderRadius: 18, height: 64, justifyContent: 'center', width: 62 },
  tileText: { color: '#FFFFFF', fontSize: 34, fontWeight: '900' },
  nextButton: { alignItems: 'center', backgroundColor: '#2FA866', borderBottomColor: '#167A43', borderBottomWidth: 5, borderRadius: 18, paddingHorizontal: 22, paddingVertical: 16 },
